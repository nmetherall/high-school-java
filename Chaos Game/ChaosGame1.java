...
class ChaosGame1 implements KeyListener
{
	...
	public void change(double x,double y, double z)
	{
		...
		{
			z = updateZ(.10625,.005,0);
			//y = update(-.04,.85,1.6);
		}

		else if(.86<r && r <= .93)
		{
			z = updateZ(.025,-.0325,0);
			y = update(.23,.22,1.6);
		}

		else if(.93< r && r <= 1)
		{
			z = updateZ(-.01875,.035,0);
			//y = update(.26,.24,.44);
		}
	...
	{
		ChaosGame1 g = new ChaosGame1();
	}//end of main()
}//end of class ChaosGame1
...
class SnowV6_5 implements MouseListener
{
	double force[][];
	int weight[][];
	boolean pos[][];
...
//	double sizeW2;
	...
	void change(int m, int n)
	{
		...
		//for(int k = 0; Math.abs(k)<=Math.abs(m); k+=e)
		{
			...
			else if(n!=0 && m==0  && pos[i+e][j+n]!=true && j+n>1 && j+n<497 && i+1<497 && i+1>1)
			{
				pos[i][j]=false;
				pos[i+e][j+n] = true;
				//prev[i][j] = false;
				prev[i+e][j+n] = true;
				force[i+e][j+n] = force[i][j];
				weight[i+e][j+n] = weight[i][j];
				partCol[i+e][j+n] = partCol[i][j];
				return;
			}
			else if(m!=0 && n==0)
			{
				...
				{
					if(pos[i+k+e][j] != true);
					else if(pos[i+k+e][j] == true && j+n>1 && j+n<497 && i+k<497 && i+k>1)
					{

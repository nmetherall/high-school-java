...
class SnowV6_2 implements MouseListener
{
	double force[][];
	int weight[][];
	boolean pos[][];
...
//	double sizeW2;
	...
	SnowV6_2()
	{
		...
		ran = new Checkbox("Random Color", false);
		ran.setBounds(20,220,150,20);
		frame.add(ran);
			...
			for(j=499;j>0;j--)
			{
				...
				if(pos[i][j] == true)
				{
					...
					if (partCol[i][j] ==1)
					{
						...
						switch(rand)
						{
							case 0: back.setRGB(i,j,BLACK);break;
							case 1: back.setRGB(i,j,GRAY);break;
							case 2: back.setRGB(i,j,BLUE);break;
							case 3: back.setRGB(i,j,RED);break;
							case 4:  back.setRGB(i,j,GREEN);break;
							case 5: back.setRGB(i,j,CYAN);break;
							case 6:  back.setRGB(i,j,ORANGE);break;
							case 7:  back.setRGB(i,j,WHITE);break;
							case 8:  back.setRGB(i,j,PINK);break;
							case 9:  back.setRGB(i,j,YELLOW);break;
						}
					}
					else
			...
			//if(/*pos[i+n][j+m] != true &&*/ prev[i][j]!=true || Math.abs(m)+Math.abs(n)==0)
				...
				for(int k = 0; Math.abs(k)<=Math.abs(m); k+=e)
				{
					...
					else
					{
						...
						//prev[i][j] = false;
						prev[i+k-e][j+n] = true;
						force[i+k-e][j+n] = force[i][j];
						weight[i+k-e][j+n] = weight[i][j];
						partCol[i+k-e][j+n] = partCol[i][j];
						//k= Math.absabs
					}
				}
			}

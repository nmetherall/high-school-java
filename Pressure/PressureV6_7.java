...
class PressureV6_7 implements MouseListener,ActionListener
{
	boolean startVal;
	double force[][];
	int weight[][];
...
//	double sizeW2;
	...
	PressureV6_7()
	{
		...
		frame = new JFrame("Pressure V6_7");
		...
		wallB = new Checkbox("Wall", false);
		wallB.setBounds(0,0,150,20);
		frame.add(wallB);
		...
		for(i=1;i<499;i++)
			for(j=499;j>0;j--)
			{
				...
				if(pos[i][j] == true)
				{
					...
					if(j >= 499 || j<=1 || j+forced > 499 || j+forced<=1 || wall[i][j]==true)
					{
						change(0,0);
					}
					else
	...
	void change(int m, int n)
	{
		//for(int k = 0; Math.abs(k)<=Math.abs(m); k+=e)
		{
			...
			{
				for(int k = 0; Math.abs(k)<=Math.abs(forced) && Math.abs(k)<3 && j+n>1 && j+n<497 && i+k<497 && i+k>1; k+=e)
				{
					if(pos[i+k+e][j] != true);
	...
	{
		PressureV6_7 s = new PressureV6_7();
	}//end of main

...
class SnowV4 implements MouseListener
{
	double force[][];
	int weight[][];
	boolean pos[][];
...
//	double sizeW2;
	...
	SnowV4()
	{
		...
		ran = new Checkbox("Erase", false);
		ran.setBounds(20,420,150,20);
		frame.add(ran);
						...
						if(force[i][j]!=0)
						{
									...
									if(pos[i+1][j+forced]==true || i+1>=497)
									{
										...
										else
										{
											...
											else
											{
												...
												partCol[i-1][j+forced] = partCol[i][j];
												//weight[i][j]=0;
												//force[i][j] = 0;
											}
										}
							...
							else
							{
								...
								else
								{
									...
									partCol[i][j+forced] = partCol[i][j];
									//weight[i][j]=0;
									//force[i][j] = 0;
								}
							}
